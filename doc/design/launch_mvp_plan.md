...
| `RESPONSIVE-001` | Adaptive UI across supported sizes and inputs | Done for recorded bounded checks; shell no longer forces 760px page min-width (390px path); ≤360px shell/Arena/paperdoll compression; short-landscape shell/Arena compression; Shop/Arena/shell/Inventory/city/allocation/profile/quests coarse-pointer 44px controls; Shop empty/licenses soften below 520px; location tabs wrap below 520px; expanded standard audit still open | Gameplay fidelity and adaptive UI matrix |
...
| `CITY-SERVICES-001` | Complete building/service interiors | Partially Done — Ashen desks ship; schools/clan/post/library/guard/forge/tavern-rumors recovery CTAs; Auction/Market stalls/Numismatics explicitly deferred with live-loop CTAs; unfinished landmarks keep City/Shop/Hall fallback | Gameplay fidelity and adaptive UI matrix |
...
| Player profile — public/alternate states | Public lookup, non-owner controls, filled equipment, and saved/no-allocation states. | Canonical `/player/:name` public and alternate owner views. | **Partially Implemented** | Public lookup, Assault CTA, empty public-perks note, owner idle-allocation note, and sheet empty-NV/VM/vault/fatigue/alignment recoveries ship. Remaining visual parity vs live Neverlands still needs evidence. |
