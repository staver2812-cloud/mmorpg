...
| 2026-09-13 | Equip requirement failures (`broken` / `expired` / unmet stats) use `game.inventory.wear_item_*` / `requirements_not_met`; equip/unequip structural rejects use `game.inventory.item_not_equipment` / `item_already_equipped` / `invalid_equipment_slot` / `slot_not_specified` / `no_item_in_slot`. |
